...
<head>
    ...
    <#assign wantokTitlePrefix = "WanERP">
    <#--<title>${layoutSettings.companyName}: <#if (titleProperty)?has_content>${uiLabelMap[titleProperty]}<#else>${title!}</#if></title>-->
    <title>${wantokTitlePrefix}<#if (titleProperty)?has_content>: ${uiLabelMap[titleProperty]}<#else>${title!}</#if></title>
...
<link rel="stylesheet/less" href="<@ofbizContentUrl>/wanerptheme/${visualTheme.visualThemeId}.less</@ofbizContentUrl>" type="text/css"/>
